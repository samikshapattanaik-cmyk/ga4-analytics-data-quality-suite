...
def audit_url(row_id, raw_url, taxonomy):
    ...
    # or a required param is genuinely missing — in both cases there's no value we
    # can respons­ibly fill in automatically, so a "corrected" URL would just be a
    # no-op copy that looks like a failed fix rather than an honest "needs input") ---
    has_unfillable = any(has_placeholder for _, _, has_placeholder in corrected_pairs) or missing_required
    if has_unfillable:
        corrected_url = None
    else:
